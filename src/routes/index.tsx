...
app.get('/following-feed', (c) => {
  ...
  const filter = c.req.query('filter') || 'all' // all | voice_journal | song
        ...
        <div class="mb-6">
          ...
          <div class="flex gap-2">
            {[
              ...
              >
                {tab.label}
                {tab.key === 'all' && <span class="ml-1.5 text-xs opacity-70">{dummyFeed.length}</span>}
                {tab.key === 'voice_journal' && <span class="ml-1.5 text-xs opacity-70">{dummyFeed.filter(i => i.type === 'voice_journal').length}</span>}
                {tab.key === 'song' && <span class="ml-1.5 text-xs opacity-70">{dummyFeed.filter(i => i.type === 'song').length}</span>}
              </a>
            ))}
          </div>
        </div>
                      ...
                      <div>
                        ...
                        <div class="bg-brand-50 rounded-xl p-3">
                          <div class="flex items-center gap-3">
                            <button id={`play-btn-${vj.id}`} class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-white" style="background:#3085c7" onclick={`initAudioPlayer('${vj.id}', '')`}>
                        ...
                        {vj.tags?.length > 0 && (
                          <div class="flex flex-wrap gap-1.5 mt-2">
                            {vj.tags.map((tag: string) => (
                              <span class="tag-badge text-xs px-2 py-0.5 rounded-full">#{tag}</span>
                            ))}
                          </div>
                        )}
                      </div>
...
app.get('/', (c) => {
  ...
  // おすすめ投稿（フォロー外のユーザーの人気投稿をダミーで表示）
  const recommendedVJs = dummyVoiceJournals.slice(2, 5)
          ...
          <div class="relative overflow-hidden rounded-2xl" id="bannerCarousel">
            ...
            <div class="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
              {dummyBanners.map((_, i) => (
                <button onclick={`setBanner(${i})`} class={`w-2 h-2 rounded-full transition-all ${i === 0 ? 'bg-white w-4' : 'bg-white/50'}`} id={`dot-${i}`}></button>
              ))}
            </div>
          </div>
        </div>
      ...
      <div class="max-w-4xl mx-auto px-4 py-8 space-y-12">
        ...
        <section>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden card-hover audio-card">
                  <div class="flex items-center gap-3 p-4 pb-0">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <p class="text-xs text-gray-500">
                        {item.type === 'voice_journal' ? 'ボイスジャーナルを投稿しました' : 'ここロザシソングを公開しました'}
                      </p>
                    </div>
                  </div>
                  <div class="p-4">
          ...
          <div class="text-center mt-6">
            <a href="/following-feed" class="btn-outline px-8 py-3 rounded-full text-sm font-medium inline-block">
              もっと見る
            </a>
          </div>
        </section>
            ...
            {recommendedVJs.map(vj => (
              <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden card-hover audio-card">
                <div class="flex items-center gap-3 p-4 pb-0">
                  ...
                  <span class="text-xs text-brand-600 bg-brand-50 px-2 py-1 rounded-full font-medium">おすすめ</span>
                </div>
                <div class="p-4">
                  <a href={`/voice-journal/${vj.id}`} class="block mb-3">
                    <h3 class="font-bold text-gray-800 hover:text-brand-600 line-clamp-2">{vj.title}</h3>
                  ...
                  <div class="bg-brand-50 rounded-xl p-3">
                    <div class="flex items-center gap-3">
                      <button id={`play-btn-rec-${vj.id}`} class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-white" style="background:#3085c7" onclick={`initAudioPlayer('rec-${vj.id}', '')`}>
                        <i class="fas fa-play text-sm"></i>
                      </button>
                      <div class="flex-1">
                        <div class="bg-gray-200 rounded-full h-1.5">
                          <div id={`progress-rec-${vj.id}`} class="audio-player-bar h-1.5 rounded-full" style="width:0%"></div>
                        </div>
                        <div class="flex justify-between text-xs text-gray-400 mt-1">
                  ...
                  {vj.tags?.length > 0 && (
                    <div class="flex flex-wrap gap-1.5 mt-2">
                      {(vj.tags as string[]).map((tag: string) => (
                        <span class="tag-badge text-xs px-2 py-0.5 rounded-full">#{tag}</span>
                      ))}
                    </div>
                  )}
                ...
                <div class="px-4 pb-4 flex items-center gap-1 flex-wrap">
                  {Object.entries(reactionLabels).slice(0, 3).map(([type, info]) => (
                    <button onclick={`toggleReaction(this, '${type}')`} class="reaction-btn flex items-center gap-1 text-xs text-gray-500 hover:text-brand-600 bg-gray-50 px-2 py-1 rounded-full border border-gray-100">
                      <span>{info.emoji}</span>
        ...
        <section>
          <div class="flex items-center justify-between mb-6">
            ...
            <a href="/voice-journals" class="text-sm text-brand-600 hover:underline">もっと見る</a>
          </div>
              ...
              <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden card-hover audio-card">
                <div class="p-4">
                  ...
                  <div class="bg-brand-50 rounded-xl p-2.5 flex items-center gap-2">
                    <button id={`play-btn-${vj.id}-card`} class="w-8 h-8 rounded-full flex items-center justify-center text-white flex-shrink-0" style="background:#3085c7" onclick={`initAudioPlayer('${vj.id}-card', '')`}>
                      <i class="fas fa-play text-xs"></i>
                    </button>
                    <div class="flex-1">
                      <div class="bg-white/60 rounded-full h-1">
                  ...
                  <div class="flex items-center gap-1 mt-3 flex-wrap">
                    {Object.entries(reactionLabels).slice(0, 3).map(([type, info]) => (
                      <button onclick={`toggleReaction(this, '${type}')`} class="reaction-btn flex items-center gap-1 text-xs text-gray-500 hover:text-brand-600 bg-gray-50 px-2 py-1 rounded-full">
                        <span>{info.emoji}</span>
                        <span class="reaction-count">{vj.reactions_count?.[type as keyof typeof vj.reactions_count] || 0}</span>
                      </button>
                    ))}
        ...
        {/* ── 今の気持ちを声に（新着VJの直下） ── */}
        <section class="bg-brand-50 rounded-2xl border border-brand-100 p-6">
        ...
        <section>
          <div class="flex items-center justify-between mb-6">
            ...
            <a href="/columns" class="text-sm text-brand-600 hover:underline">もっと見る</a>
          </div>
        ...
        {/* ── Bottom CTA ── */}
        <section class="rounded-3xl overflow-hidden" style="background:#3085c7">
